...
export default function PostEditor({ post }: Props) {
  ...
  const [type, setType] = useState<"study" | "task" | "blog">(post.type ?? "study");
  const [status, setStatus] = useState<"pending" | "done">(post.status ?? "pending");
    ...
    <div className="flex flex-col gap-8 max-w-[900px] mx-auto w-full">
      ...
      <div className="flex justify-end gap-4 pt-6 pb-12">
        ...
        <div className="flex flex-col md:flex-row justify-between mb-4 gap-4 overflow-hidden w-full onload-animation">
            <a 
            href={`/posts/${post.slug}/`}  className="w-full font-bold overflow-hidden active:scale-95">
                <div className="btn-card rounded-2xl w-full h-[3.75rem] px-4 flex items-center justify-start gap-4">
                    <Icon name="material-symbols:chevron-left-rounded" className="text-[2rem] text-[var(--primary)]" icon={""} />
                    <div className="text-black/75 dark:text-white/75">Volver al inicio</div>
                </div>
            </a>
             <button
          onClick={handleSave}
