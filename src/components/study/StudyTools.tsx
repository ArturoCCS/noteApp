...
export default function StudyTools({ content }: { content: string }) {
  const [active, setActive] = useState<"summary" | "flashcards" | "questions">("summary");
  ...
  return (
    <div className="card-base px-6 py-6 mt-10">
      ...
      <div className="flex gap-3 mb-6">
        <Tab label={`Resumen (${parsed.summary.length})`} active={active === "summary"} onClick={() => setActive("summary")} />
        <Tab label={`Tarjetas (${parsed.flashcards.length})`} active={active === "flashcards"} onClick={() => setActive("flashcards")} />
        <Tab label={`Preguntas (${parsed.questions.length})`} active={active === "questions"} onClick={() => setActive("questions")} />
      </div>

      {active === "summary" && <StudySummary bullets={parsed.summary} />}
      {active === "flashcards" && <Flashcards cards={parsed.flashcards} />}
      {active === "questions" && <StudyQuestions questions={parsed.questions} />}
    </div>
  );
}
