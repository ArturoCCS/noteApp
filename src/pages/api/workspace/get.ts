...
export const GET: APIRoute = async ({ request }) => {
  ...
  if (!auth.workspaceSlug) {
    return new Response(JSON.stringify({ error: "Missing workspaceSlug custom claim" }), { status: 400 });
  }

  const ws = await getWorkspaceBySlug(auth.workspaceSlug);
  return new Response(JSON.stringify({ ok: true, workspace: ws }), { status: 200 });
};